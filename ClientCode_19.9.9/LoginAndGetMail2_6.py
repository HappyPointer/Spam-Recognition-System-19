...
def send_client_email_list(intensity,emaillist,times=2.5):
    response_result_list=[]
    email_content_list=[]
    for oneemail in emaillist:
        email_content_list.append({'body':oneemail[1],'intensity':intensity})
    judge_result_list=send_client('request-result',email_content_list,times)
    num=0
...
def checkAndJudgeOldMail(serv,intensity,rulerlist,times=2.5):
    ...
    for oneemail in emaillist:
        ...
        if response_result != []:
            response_result_list.append(response_result)
    try:
        response_result_list2 = send_client_email_list(intensity,email_list,times)
    except:
        response_result_list2 = []
        for email in email_list:
            response_result = []
