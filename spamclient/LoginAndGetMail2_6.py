...
def send_client_email_list(emaillist):
    response_result_list=[]
    email_content_list=[]
    for oneemail in emaillist:
        email_content_list.append({'body':oneemail[1],'intensity':'low'})
...
def checkAndJudgeOldMail(serv,rulerlist):
    """
    获取邮箱内的历史邮件并给出其判断结果
    :param serv:
    :param rulerlist:
...
def judgeNewMail(emaillist,rulelist):
    """
    判断一个列表的邮件是否为垃圾邮件
    :param emaillist:
    :param rulelist:
